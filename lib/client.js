...
login(username, password) {
	...
	return new Promise((fulfill, reject) => {
		...
		if (!this.useAlternateLoginApi) {
			...
			this.client.post('https://elements.cloud.sengled.com/zigbee/customer/login.json',
			{
				'uuid': this.uuid,
				'user': username,
				'pwd': password,
  ...
  getDevices() {
		...
		return new Promise((fulfill, reject) => {
			this.client.post('https://elements.cloud.sengled.com/zigbee/device/getDeviceDetails.json', {})
				.then((response) => {
  ...
  userInfo() {
    ...
    return new Promise((fulfill, reject) => {
      this.client.post('https://elements.cloud.sengled.com/zigbee/customer/getUserInfo.json', {})
      .then((response) => {
        if (response.data.ret == 100) {
          reject(response.data);
        } else {
  ...
  deviceSetOnOff(deviceId, onoff) {
    ...
    return new Promise((fulfill, reject) => {
      this.client.post('https://elements.cloud.sengled.com/zigbee/device/deviceSetOnOff.json', {"onoff": onoff ? 1 : 0,"deviceUuid": deviceId})
      .then((response) => {
        if (response.data.ret == 100) {
          reject(response.data);
        } else {
  ...
  deviceSetBrightness(deviceId, brightness) {
    return new Promise((fulfill, reject) => {
        this.client
            .post('https://elements.cloud.sengled.com/zigbee/device/deviceSetBrightness.json', {
                brightness: brightness,
                deviceUuid: deviceId
            })
            .then(response => {
...
deviceSetColorTemperature(deviceId, colorTemperature) {
    return new Promise((fulfill, reject) => {
        this.client
            .post('https://elements.cloud.sengled.com/zigbee/device/deviceSetColorTemperature.json', {
...
deviceSetRgbColor(deviceId, rgb) {
    return new Promise((fulfill, reject) => {
        this.client
            .post('https://elements.cloud.sengled.com/zigbee/device/deviceSetGroup.json', {
		cmdId: 129,
